...
static uint8_t get_threads_and_width(uint8_t* width ,uint8_t num_class){
	uint8_t num_procs=get_nprocs();
	uint8_t num_threads;
	...
	if(num_class<=num_procs){
		...
		*width = (uint8_t)ceil((double)(num_class/(double)num_threads));
	}
...
uint16_t get_slice_of_G( double* G_s, uint64_t* seeds, uint16_t num_seeds, double tel_prob, csr_graph graph ){
	...
	csr_graph* graph_copies = csr_mult_deep_copy( graph_scaled, (uint8_t) NUM_THREADS );	
...
void* my_power_iter(void* param){
	...
	*(data->iter)=iter;
	...
	printf("Thread from column %"PRIu16" to %"PRIu16" finished in %lf sec \n", data->from, data->to, time_spent);
...
void my_PPR_single_thread( double* soft_labels, csr_graph graph, uint16_t num_seeds,
			   const uint64_t* seed_indices, uint8_t num_class, uint8_t* class_ind,
