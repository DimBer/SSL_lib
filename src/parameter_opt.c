...
//	step_size = STEPSIZE_2/pow(frob_norm(A,K), 2.0f);
	...
	for(uint16_t i=0;i<K;i++) x[i]=1.0f/(double)K;
	
	//Initialize to e_k
	for(uint16_t i=0;i<K;i++) x[i] = (i==K-1) ? 1.0f : 0.0f;
...
void hyperplane_constr_QP(double* x, double* A, double* b, uint16_t K){ 
	...
	for(uint16_t i=0;i<K;i++) *(A_temp+i)=&A[i*K];
				

        for(uint16_t i=0;i<K;i++) A_temp[i][i]+= L2_REG_LAMBDA;  //L2 regularization is needed due to bad conditioning of A
	...
	for(uint16_t i=0;i<K;i++){
		printf("\n");
		for(uint16_t j=0;j<K;j++) printf(" %lf ",A_temp[i][j]);
	}			
	printf("\n B: ");
	for(uint16_t i=0;i<K;i++) printf(" %lf ",b[i]);

	for(uint16_t i=0;i<K;i++){
		b[i]=1.0;
		for(uint16_t j=0;j<K;j++) A_temp[i][j]=0.0;
	...
	//The final solution
	for(uint16_t i=0;i<K;i++) x[i] = x_unc[i] - x_all_ones[i];
...
static uint16_t tune_parameters( double* x, double* G_ll,  uint8_t* ind , uint16_t L,
	...
	for(uint16_t i=0;i<L;i++){ 
		if(ind[i]==1){
			for(uint16_t j=0;j<L;j++) G_ll_signed[j*L +i] = G_ll[j*L +i] ;
		}else{
			for(uint16_t j=0;j<L;j++) G_ll_signed[j*L +i] = - G_ll[j*L +i] ;
		}
	}
	...
	do{
		...
		for(uint16_t j=0;j<L;j++) res[j] = (ind[j]==1)? res[j]-1 : res[j]+1 ; 
	...
	for(uint16_t j=0;j<L;j++) x[j]=(ind[j]==1)? x[j] : -x[j] ;  //Give back the signs to theta
...
void tune_all_parameters(double* theta,double* G_ll, uint8_t* class_ind, uint16_t num_seeds,
	...
	for(uint16_t i=0;i<num_class;i++){
		...
		for(uint16_t j=0;j<num_seeds;j++) theta[j*num_class + i]=this_theta[j];
